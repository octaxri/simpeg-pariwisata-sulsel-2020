    ...
    function upload_image($file, $method, $folder, $image_name, $error_data, $required = FALSE)
    {
        ...
        if ( ! empty($_FILES[$file]['tmp_name']))
        {
            ...
            if ( ! $CI->upload->do_upload($file))
            {
                ...
                $CI->load->view('admin/_layout/wrapper', $error_data);
                return FALSE;
            }
...
if ( ! function_exists('upload_allTypeFile'))
{
    function upload_allTypeFile($file, $method, $folder, $document_name, $ajax, $error_data = null)
    {
        $CI = get_instance();
        // Bypass jika tidak mengupload file ketika mengedit data
        if ($method == 'edit')
        {
            if (empty($_FILES[$file]['tmp_name'])) return array("success" => TRUE, "dokumen" => $document_name,"error" => null);
        }

        $config['upload_path']      = './'.upload_path($folder);
        $config['allowed_types']    = 'pdf|doc|docx|xls|xlsx|ppt|pptx|jpg|png|gif';
        $config['max_size']         = '12000'; // KB
        $CI->load->library('upload', $config);
        if ( ! $CI->upload->do_upload($file))
        {
            //Jika model uploadnya ajax
            $error_data['error_upload'] = $CI->upload->display_errors('<i style="color: red;">', '</i>');
            if($ajax)
                return array("success" => FALSE, "dokumen" => "","error" => $error_data['error_upload']);
        ...
        else
        {
            ...
            if($ajax)
                return array("success" => TRUE, "dokumen" => $dokumen,"error" => null);
            return $dokumen;
        }
    }
}
...
if ( ! function_exists('view_error'))
{
    function view_error($judul = 'Error 404', $error_type = 404)
    {
        ...
        $data = array(  'title'     => $judul,
                        'jml'       => jml_nav(),
                        'isi'       => 'admin/_error/'.(string)$error_type);
        $CI->load->view('admin/_layout/wrapper', $data);
        return;
    }
}
...
if ( ! function_exists('check_mpp_label'))
{
    function check_mpp_label($month)
    {
        switch ($month) {
            case '12':
                return 'default';

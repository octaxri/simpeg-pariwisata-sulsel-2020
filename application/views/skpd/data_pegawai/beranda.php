<a class="btn btn-primary btn-sm" href="<?=skpd_url('pegawai/tambah')?>" type="button"><i class="fa fa-plus"></i>Tambah Data Pegawai</a>
<hr>
  ...
  <div class="col-md-12">
    <div class="panel">
      ...
      <div class="panel-body">
        <p>Silahkan isi opsi yang diinginkan untuk menampilkan data secara spesifik.</p>
        <hr>
        <div class="row">
          <div class="col-md-12">
          ...
          <div class="col-md-12">
            ...
            <div class="form-group col-sm-8">
              ...
              <select id="satker" class="form-control select-2" disabled readonly>
                <option value="all">-Semua Bidang/Bagian-</option>
                <?php foreach ($data_satker as $data_satker): ?>
                  <option value="<?=$data_satker->nama_satker?>" <?=$data_satker->nama_satker == $this->session->fullname ? 'selected' : '' ?>><?=$data_satker->nama_satker?></option>
                <?php endforeach ?>
              </select>
            </div>
          </div>
...
function hapusData(id) {
    ...
    function(isConfirm){
      if (isConfirm) {
        $.ajax({
            type: 'POST',
            url: '<?=skpd_url('DataPegawai/hapusAjax/')?>'+id
...
<script type="text/javascript">
    ...
    $.ajax({
        type: 'POST',
        url: '<?=skpd_url("DataPegawai/FilterAjax/")?>',
        data: {
                    ...
                    for(var i =0;i <= response.data.length-1;i++)
                    {
                      ...
                      var div = div+ `
                                  <tr data-id="`+item.nip+`">
                                    ...
                                    <td>
                                        <a href="<?=skpd_url('DataPegawai/profil/')?>`+item.nip+`" class="btn btn-success btn-xs  btn-block" > <i class="fa fa-eye"></i> Profil</a>
                                        <a href="<?=skpd_url('DataPegawai/edit/')?>`+item.nip+`" class="btn btn-primary btn-xs btn-block" > <i class="fa fa-pencil"></i> Edit </a>
                                        <button  title="Delete" class="btn btn-danger btn-xs btn-block" onclick="hapusData('`+item.nip+`');"> <i class="fa fa-trash"></i> Hapus </button>
                                    </td>
                                  </tr>
                      `;
                      $("#ajaxResult").html

...
class DataPegawai extends Admin_controller
{
    ...
    public function index()
    {
        ...
        $data = array(
            ...
            'isi'       => 'admin/data_pegawai/beranda'
        );
    ...
    public function tambahAjax()
    {
        ...
        $valid->set_rules('tmt_jenis_pangkat', 'Field TMT Jenis Pangkat', 'required|trim|strip_tags|htmlspecialchars');
        ...
        if ($valid->run() === TRUE) {
            ...
            if ($gambar !== FALSE) {
                $data_pegawai = array(
                    ...
                    'tmt_jenis_pangkat'      => $input['tmt_jenis_pangkat'],
                ...
                if ($detail) {
                    ...
                    if ($detail_user) {
                        ...
                        redirect(admin_url('DataPegawai/profil/') . $input['nip']);
                    } else {
                        $this->session->set_flashdata('gagal', 'Data Pegawai gagal ditambah.');
                        redirect(admin_url('DataPegawai/tambah'));
                    }
                } else {
                    $this->session->set_flashdata('gagal', 'Data Pegawai gagal ditambah.');
                    redirect(admin_url('DataPegawai/tambah'));
    ...
    public function edit($nip = NULL)
    {
        ...
        $data_keteranganlainnya = $this->crud->gda('keterangan_lainnya', array('nip' => $nip));
        $data_keteranganbadan = $this->crud->gda('keterangan_badan', array('nip' => $nip));
    ...
    public function editAjax($nip = NULL)
    {
        ...
        if ($valid->run() === TRUE) {
            ...
            if ($detail) {
                ...
                redirect(admin_url('DataPegawai'));
            } else {
    ...
    public function filterAjax()
    {
        ...
        if ($valid->run() === TRUE) {
            ...
            $this->datatables->order_by("SUBSTRING_INDEX(jenis_pangkat,'-',-1) DESC");
    ...
    public function exportexcel()
    {
        ...
        $a['satker'] = $this->db->distinct()
            ...
            ->join('keterangan_lainnya d', 'd.nip=a.nip', 'left')
            ->where('a.satuan_kerja', $satker)
            ->order_by("CASE WHEN a.eselon = 'Non-Eselon' THEN 1 ELSE 0 END ASC")
            ->order_by("a.eselon ASC")
            ->order_by("SUBSTRING_INDEX(a.jenis_pangkat,'-',-1) DESC")
            ->get()
			->result();
		foreach ($a['satker'] as $item) {
			if(is_numeric($item->jenis_pangkat)){
				$data = $this->crud->gd("data_golongan", array("id_golongan" => $item->jenis_pangkat));
				$item->jenis_pangkat = $data->nama_pangkat . " - " . $data->nama_golongan . " " . $data->nama_ruang;
			}
		}
        $this->load->view('admin/data_pegawai/exportexcel', $a);
    }
}
